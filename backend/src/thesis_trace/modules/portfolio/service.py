...
from thesis_trace.modules.portfolio.trades import allocate_company_action, canonical_trade_fingerprint, validate_allocations
...
def _digest(value: object) -> str:
    encoded = json.dumps(asdict(value), default=_json_default, sort_keys=True, separators=(",", ":"), ensure_ascii=False)
...
class PortfolioService:
    ...
    def save_cost_profile(self, command: SaveCostProfileCommand, *, now: datetime) -> PortfolioRecord:
        self._authorize(command.actor)
        current = self._store.get(command.actor.actor_id)
        profile = CostProfileSnapshot(
            version=1 if current is None or current.cost_profile is None else current.cost_profile.version + 1,
    ...
    def save_investable_cash(self, command: SaveInvestableCashCommand, *, now: datetime) -> PortfolioRecord:
        current = self.get(command.actor)
        if not command.cash.is_finite() or command.cash < 0 or command.as_of.tzinfo is None:
            raise ValueError("invalid_cash")
        record = replace(current, version=command.expected_version + 1, cash=command.cash, cash_as_of=command.as_of, updated_at=now)
        return self._commit(command, record, "investable_cash_saved")

    def preview_trade(self, command: PreviewTradeCommand, *, now: datetime | None = None) -> TradePreview:
        ...
        holdings = list(self._refresh_official_holdings(command.actor.actor_id, current.holdings, evaluation_time))
        official = self._official_security(command.actor.actor_id, command.trade.security_id, evaluation_time)
        current_snapshot = PortfolioSnapshot(current.version, current.cash, tuple(holdings))
        ...
        for allocation in command.allocations:
            delta = Decimal(allocation.quantity) * (Decimal(1) if command.trade.side is TradeSide.BUY else Decimal(-1))
            index = next(
                (
                    index
                    for index, value in enumerate(holdings)
                    if value.bucket_id == allocation.bucket_id and value.security_id == command.trade.security_id
                ),
                None,
            )
            if index is None:
                ...
                holdings.append(HoldingSnapshot(
                    allocation.bucket_id, command.trade.security_id, delta,
                    official.official_close, official.official_industry, official.themes,
                    official.price_date, official.price_source, official.industry_source,
                    official.classification_effective_at, official.theme_snapshot_version,
                    official.theme_source, official.theme_effective_at,
                ))
            else:
                ...
                holdings[index] = replace(
                    holdings[index], quantity=updated, official_close=official.official_close,
                    official_industry=official.official_industry, themes=official.themes,
                    price_date=official.price_date, price_source=official.price_source,
                    ...
                    theme_source=official.theme_source, theme_effective_at=official.theme_effective_at,
                )
        post = PortfolioSnapshot(current.version + 1, post_cash, tuple(value for value in holdings if value.quantity > 0))
        ...
        if command.trade.side is TradeSide.SELL and not self._sell_reduces_or_preserves_breaches(current_exposure, exposure):
            raise ValueError("risk_cap")
        preview_body = {
        ...
        preview_digest = sha256(json.dumps(
            preview_body, default=_json_default, sort_keys=True, separators=(",", ":")
        ).encode()).hexdigest()
        return TradePreview(
            current.version, fingerprint, preview_digest, command.trade, command.allocations,
            post, exposure, official,
        )

    def _official_security(
        self, actor_id: str, security_id: str, evaluation_time: datetime,
    ) -> OfficialSecuritySnapshot:
    ...
    def _refresh_official_holdings(
        self, actor_id: str, holdings: tuple[HoldingSnapshot, ...], evaluation_time: datetime,
        ...
        for holding in holdings:
            ...
            refreshed.append(replace(
                holding, official_close=fact.official_close,
                official_industry=fact.official_industry, themes=fact.themes,
                price_date=fact.price_date, price_source=fact.price_source,
                ...
                theme_source=fact.theme_source, theme_effective_at=fact.theme_effective_at,
            ))
    ...
    def _sell_reduces_or_preserves_breaches(before, after) -> bool:
        for prefix, before_values, after_values in (
            ("security_cap_exceeded:", before.security_exposure, after.security_exposure),
            ("industry_cap_exceeded:", before.industry_exposure, after.industry_exposure),
            ("theme_cap_exceeded:", before.theme_exposure, after.theme_exposure),
        ):
            breached_after = {
                reason.removeprefix(prefix)
                for reason in after.reasons if reason.startswith(prefix)
            }
    ...
    def confirm_trade(self, command: ConfirmTradeCommand, *, now: datetime) -> PortfolioRecord:
        current = self.get(command.actor)
        ...
        if canonical_trade_fingerprint(command.preview.trade) != command.preview.fingerprint:
            raise ValueError("preview_mismatch")
        if any(value.fingerprint == command.preview.fingerprint for value in current.trades):
            raise ValueError("duplicate_trade")
        trade_record = TradeRecord(
            trade_id=self._trade_id_factory(), version=1, fingerprint=command.preview.fingerprint,
            trade=command.preview.trade, allocations=command.preview.allocations, confirmed_at=now,
            reason=_required(command.reason, "reason_required"),
        )
        changed = replace(
            current, version=current.version + 1, cash=command.preview.post_portfolio.cash,
            holdings=command.preview.post_portfolio.holdings, trades=current.trades + (trade_record,), updated_at=now,
        )
        return self._commit(command, changed, "trade_confirmed")

    def preview_correction(
        self, actor: PortfolioActorContext, *, expected_version: int,
        original_trade_id: str, now: datetime,
    ) -> TradeCorrectionPreview:
        current = self.get(actor)
        if current.version != expected_version:
            raise ValueError("version_conflict")
        if any(value.original_trade_id == original_trade_id for value in current.corrections):
            raise ValueError("trade_already_corrected")
        original = next((value for value in current.trades if value.trade_id == original_trade_id), None)
        if original is None:
            raise LookupError("resource_unavailable")
        if current.trades[-1].trade_id != original_trade_id or any(
            value.confirmed_at >= original.confirmed_at for value in current.company_actions
        ):
            raise ValueError("correction_requires_reconciliation")
        holdings = list(self._refresh_official_holdings(actor.actor_id, current.holdings, now))
        cash = current.cash
        ...
        for allocation in original.allocations:
            ...
            index = next((
                index for index, value in enumerate(holdings)
                if value.bucket_id == allocation.bucket_id and value.security_id == original.trade.security_id
            ), None)
            if index is None:
                if delta < 0:
                    raise ValueError("correction_holding_conflict")
                holdings.append(HoldingSnapshot(
                    allocation.bucket_id, original.trade.security_id, delta,
                    fact.official_close, fact.official_industry, fact.themes,
                    fact.price_date, fact.price_source, fact.industry_source,
                    fact.classification_effective_at, fact.theme_snapshot_version,
                    fact.theme_source, fact.theme_effective_at,
                ))
            else:
        ...
        post = PortfolioSnapshot(current.version + 1, cash, tuple(value for value in holdings if value.quantity > 0))
        exposure = aggregate_exposure(post)
        digest = sha256(json.dumps({
            "expected_version": current.version, "original_trade_id": original_trade_id,
            "post_cash": str(cash), "holdings": [asdict(value) for value in post.holdings],
        }, default=_json_default, sort_keys=True, separators=(",", ":")).encode()).hexdigest()
        return TradeCorrectionPreview(current.version, original_trade_id, digest, post, exposure)

    def confirm_correction(
        self, command: ConfirmTradeCorrectionCommand, *, now: datetime,
    ) -> PortfolioRecord:
        current = self.get(command.actor)
        if current.version != command.preview.expected_portfolio_version:
            raise ValueError("version_conflict")
        if not command.confirmation_id:
            raise PermissionError("confirmation_required")
        if any(value.original_trade_id == command.preview.original_trade_id for value in current.corrections):
            raise ValueError("trade_already_corrected")
        original = next((value for value in current.trades if value.trade_id == command.preview.original_trade_id), None)
        if original is None:
            raise LookupError("resource_unavailable")
        correction = TradeCorrectionRecord(
            self._correction_id_factory(), 1, original.trade_id, original.allocations,
            now, _required(command.reason, "reason_required"),
        )
        changed = replace(
            current, version=current.version + 1, cash=command.preview.post_portfolio.cash,
            holdings=command.preview.post_portfolio.holdings,
            corrections=current.corrections + (correction,), updated_at=now,
        )
        return self._commit(command, changed, "trade_corrected")

    def preview_company_action(
        self, actor: PortfolioActorContext, *, expected_version: int, security_id: str,
        action_kind: str, confirmed_post_action_shares: int, cash_in_lieu: Decimal, now: datetime,
    ) -> CompanyActionPreview:
        current = self.get(actor)
        if current.version != expected_version:
            raise ValueError("version_conflict")
        ...
        if not relevant or any(value.quantity != value.quantity.to_integral_value() for value in relevant):
            raise ValueError("invalid_company_action")
        ...
        for index, holding in enumerate(holdings):
            if holding.security_id == security_id:
                holdings[index] = replace(holding, quantity=Decimal(allocation.shares[holding.bucket_id]))
        post = PortfolioSnapshot(
            current.version + 1, current.cash + cash_in_lieu,
            tuple(value for value in holdings if value.quantity > 0),
        )
        exposure = aggregate_exposure(post)
        digest = sha256(json.dumps({
            "expected_version": current.version, "security_id": security_id,
            "action_kind": action_kind, "post_shares": confirmed_post_action_shares,
            "cash_in_lieu": str(cash_in_lieu), "allocation": asdict(allocation),
        }, default=_json_default, sort_keys=True, separators=(",", ":")).encode()).hexdigest()
        return CompanyActionPreview(
            current.version, security_id, action_kind, confirmed_post_action_shares,
            cash_in_lieu, allocation, digest, post, exposure,
        )

    def confirm_company_action(
        self, command: ConfirmCompanyActionCommand, *, now: datetime,
    ) -> PortfolioRecord:
        current = self.get(command.actor)
        if current.version != command.preview.expected_portfolio_version:
            raise ValueError("version_conflict")
        ...
        action = CompanyActionRecord(
            self._company_action_id_factory(), 1, command.preview.security_id,
            command.preview.action_kind,
            sum(int(value.quantity) for value in current.holdings if value.security_id == command.preview.security_id),
            command.preview.confirmed_post_action_shares, command.preview.cash_in_lieu,
            command.preview.allocation, now, _required(command.reason, "reason_required"),
        )
        changed = replace(
            current, version=current.version + 1, cash=command.preview.post_portfolio.cash,
            holdings=command.preview.post_portfolio.holdings,
            company_actions=current.company_actions + (action,), updated_at=now,
        )
    ...
    def _commit(self, command, record: PortfolioRecord, action: str) -> PortfolioRecord:
        expected_version = (
            command.expected_version if hasattr(command, "expected_version")

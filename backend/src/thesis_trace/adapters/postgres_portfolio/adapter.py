...
from thesis_trace.adapters.postgres_portfolio.recommendation_snapshots import (
    _append,
    _get,
)
...
_COST_PROFILES = sa.Table(
    ...
    sa.Column("minimum_buy_fee", sa.Numeric),
    sa.Column("sell_rate", sa.Numeric),
    sa.Column("minimum_sell_fee", sa.Numeric),
...
_CASH_VERSIONS = sa.Table(
    ...
    _METADATA,
    sa.Column("owner_user_id", sa.Text),
    sa.Column("portfolio_version", sa.Integer),
    sa.Column("cash", sa.Numeric),
...
_TRADE_ALLOCATIONS = sa.Table(
    ...
    sa.Column("owner_user_id", sa.Text),
    sa.Column("trade_id", sa.Text),
    sa.Column("trade_version", sa.Integer),
...
_CORRECTIONS = sa.Table(
    ...
    _METADATA,
    sa.Column("owner_user_id", sa.Text),
    sa.Column("correction_id", sa.Text),
    sa.Column("version", sa.Integer),
    sa.Column("original_trade_id", sa.Text),
...
_CORRECTION_ALLOCATIONS = sa.Table(
    ...
    sa.Column("owner_user_id", sa.Text),
    sa.Column("correction_id", sa.Text),
    sa.Column("correction_version", sa.Integer),
    sa.Column("bucket_id", sa.Text),
    sa.Column("quantity", sa.Integer),
    schema="portfolio",
)
_COMPANY_ACTIONS = sa.Table(
    ...
    sa.Column("pre_action_shares", sa.Integer),
    sa.Column("confirmed_post_action_shares", sa.Integer),
    sa.Column("cash_in_lieu", sa.Numeric),
    sa.Column("remainder_order", ARRAY(sa.Text)),
    sa.Column("allocation_policy_version", sa.Text),
    sa.Column("confirmed_at", sa.DateTime(timezone=True)),
    sa.Column("reason", sa.Text),
    sa.Column("policy_version", sa.Text),
    schema="portfolio",
...
class PostgresPortfolioStore:
    ...
    @staticmethod
    def _lock_inputs(connection: Any, actor_id: str) -> None:
        connection.execute(
            "SELECT pg_advisory_xact_lock(hashtextextended(%s,0))",
            (f"recommendation-portfolio:{actor_id}",),
        )

    def lock_recommendation_inputs(self, actor_id: str) -> None:
        context = self._security_context_provider()
        ...
        with self._connection() as connection:
            self._lock_inputs(connection, actor_id)

    @staticmethod
    def _load(
        connection: _Connection, actor_id: str, *, for_share: bool = False
    ) -> PortfolioRecord | None:
        current_query = sa.select(
            _CURRENT_STATE.c.version,
            _CURRENT_STATE.c.cash,
            _CURRENT_STATE.c.cash_as_of,
            _CURRENT_STATE.c.updated_at,
        ).where(_CURRENT_STATE.c.owner_user_id == actor_id)
        current = connection.execute(
            current_query.with_for_update(read=True) if for_share else current_query
        ).fetchone()
        if current is None:
            return None
        profile_row = connection.execute(
            sa.select(
                _COST_PROFILES.c.version,
                _COST_PROFILES.c.buy_rate,
                _COST_PROFILES.c.minimum_buy_fee,
                _COST_PROFILES.c.sell_rate,
                _COST_PROFILES.c.minimum_sell_fee,
                _COST_PROFILES.c.tax_rate,
                _COST_PROFILES.c.effective_at,
                _COST_PROFILES.c.policy_version,
            )
            .where(_COST_PROFILES.c.owner_user_id == actor_id)
            .order_by(
        ...
        profile = (
            ...
            else CostProfileSnapshot(
                int(profile_row[0]),
                Decimal(profile_row[1]),
                Decimal(profile_row[2]),
                Decimal(profile_row[3]),
                Decimal(profile_row[4]),
                Decimal(profile_row[5]),
                profile_row[6],
                str(profile_row[7]),
            )
        )
        holdings = tuple(
            ...
            for row in connection.execute(
                sa.select(
                    _HOLDINGS.c.bucket_id,
                    _HOLDINGS.c.security_id,
                    _HOLDINGS.c.quantity,
                    _HOLDINGS.c.official_close,
                    _HOLDINGS.c.official_industry,
                    _HOLDINGS.c.themes,
                    _HOLDINGS.c.price_date,
                    _HOLDINGS.c.price_source,
                    _HOLDINGS.c.industry_source,
                    _HOLDINGS.c.classification_effective_at,
                    _HOLDINGS.c.theme_snapshot_version,
                    _HOLDINGS.c.theme_source,
                    _HOLDINGS.c.theme_effective_at,
                )
                .where(_HOLDINGS.c.owner_user_id == actor_id)
                .order_by(
                    _HOLDINGS.c.bucket_id,
                    _HOLDINGS.c.security_id,
                )
        ...
        for row in connection.execute(
            sa.select(
                _TRADES.c.trade_id,
                _TRADES.c.version,
                _TRADES.c.fingerprint,
                _TRADES.c.source_kind,
                _TRADES.c.source_row_id,
                _TRADES.c.broker_reference,
                _TRADES.c.security_id,
                _TRADES.c.side,
                _TRADES.c.quantity,
                _TRADES.c.price,
                _TRADES.c.fees,
                _TRADES.c.tax,
                _TRADES.c.executed_at,
                _TRADES.c.confirmed_at,
                _TRADES.c.reason,
                _TRADES.c.policy_version,
            )
            .where(_TRADES.c.owner_user_id == actor_id)
            .order_by(
                _TRADES.c.version,
                _TRADES.c.trade_id,
            )
            ...
            allocations = tuple(
                ...
                for item in connection.execute(
                    sa.select(
                        _TRADE_ALLOCATIONS.c.bucket_id,
                        _TRADE_ALLOCATIONS.c.quantity,
                    )
                    .where(
                    ...
                    .order_by(_TRADE_ALLOCATIONS.c.bucket_id)
                ).fetchall()
            )
        ...
        for row in connection.execute(
            sa.select(
                _CORRECTIONS.c.correction_id,
                _CORRECTIONS.c.version,
                _CORRECTIONS.c.original_trade_id,
                _CORRECTIONS.c.confirmed_at,
                _CORRECTIONS.c.reason,
                _CORRECTIONS.c.policy_version,
            )
            .where(_CORRECTIONS.c.owner_user_id == actor_id)
            .order_by(
                _CORRECTIONS.c.version,
                _CORRECTIONS.c.correction_id,
            )
            ...
            allocations = tuple(
                ...
                for item in connection.execute(
                    sa.select(
                        _CORRECTION_ALLOCATIONS.c.bucket_id,
                        _CORRECTION_ALLOCATIONS.c.quantity,
                    )
                    .where(
                    ...
                    .order_by(_CORRECTION_ALLOCATIONS.c.bucket_id)
                ).fetchall()
            )
            corrections.append(
                TradeCorrectionRecord(
        ...
        for row in connection.execute(
            sa.select(
                _COMPANY_ACTIONS.c.action_id,
                _COMPANY_ACTIONS.c.version,
                _COMPANY_ACTIONS.c.security_id,
                _COMPANY_ACTIONS.c.action_kind,
                _COMPANY_ACTIONS.c.pre_action_shares,
                _COMPANY_ACTIONS.c.confirmed_post_action_shares,
                _COMPANY_ACTIONS.c.cash_in_lieu,
                _COMPANY_ACTIONS.c.remainder_order,
                _COMPANY_ACTIONS.c.allocation_policy_version,
                _COMPANY_ACTIONS.c.confirmed_at,
                _COMPANY_ACTIONS.c.reason,
                _COMPANY_ACTIONS.c.policy_version,
            )
            .where(_COMPANY_ACTIONS.c.owner_user_id == actor_id)
            .order_by(
                _COMPANY_ACTIONS.c.version,
                _COMPANY_ACTIONS.c.action_id,
            )
            ...
            allocation_rows = connection.execute(
                ...
                .order_by(_COMPANY_ACTION_ALLOCATIONS.c.bucket_id)
            ).fetchall()
    ...
    def append_recommendation_snapshot(
        ...
        if (
            ...
            or context.role.value != "owner"
        ):
            raise PermissionError("resource_unavailable")
        with self._connection() as connection:
            _append(connection, actor_id, snapshot)
    ...
    def get_recommendation_snapshot(
        ...
        if (
            ...
            return None
        with self._connection() as connection:
            return _get(connection, actor_id, snapshot_id)
    ...
    def get_official_security(
        ...
        with self._connection() as connection:
            row = connection.execute(
                sa.select(
                    _OFFICIAL_SECURITIES.c.official_close,
                    _OFFICIAL_SECURITIES.c.price_date,
                    _OFFICIAL_SECURITIES.c.price_source,
                    _OFFICIAL_SECURITIES.c.official_industry,
                    _OFFICIAL_SECURITIES.c.industry_source,
                    _OFFICIAL_SECURITIES.c.classification_effective_at,
                    _OFFICIAL_SECURITIES.c.themes,
                    _OFFICIAL_SECURITIES.c.theme_snapshot_version,
                    _OFFICIAL_SECURITIES.c.theme_source,
                    _OFFICIAL_SECURITIES.c.theme_effective_at,
                    _OFFICIAL_SECURITIES.c.policy_version,
                ).where(
    ...
    def save_official_security(
        self, actor_id: str, snapshot: OfficialSecuritySnapshot
    ) -> None:
        ...
        with self._connection() as connection:
            ...
            values = {
                ...
                "official_industry": snapshot.official_industry,
                ...
                "theme_snapshot_version": snapshot.theme_snapshot_version,
                "theme_source": snapshot.theme_source,
                "theme_effective_at": snapshot.theme_effective_at,
            ...
            connection.execute(
                statement.on_conflict_do_update(
                    index_elements=[
                        _OFFICIAL_SECURITIES.c.owner_user_id,
                        _OFFICIAL_SECURITIES.c.security_id,
                    ],
                    set_={
                        key: getattr(statement.excluded, key)
                        for key in values
                        if key not in {"owner_user_id", "security_id"}
                    },
                )
            )

    def save_official_security_for_test(
        self, actor_id: str, snapshot: OfficialSecuritySnapshot
    ) -> None:
    ...
    @staticmethod
    def _insert_history(
        connection: _Connection, actor_id: str, record: PortfolioRecord
    ) -> None:
        if record.cost_profile is not None:
            value = record.cost_profile
            connection.execute(
                pg_insert(_COST_PROFILES)
                .values(
                    owner_user_id=actor_id,
                    version=value.version,
                    buy_rate=value.buy_rate,
                    minimum_buy_fee=value.minimum_buy_fee,
                    sell_rate=value.sell_rate,
                    minimum_sell_fee=value.minimum_sell_fee,
        ...
        connection.execute(
            pg_insert(_CASH_VERSIONS)
            .values(
                owner_user_id=actor_id,
                portfolio_version=record.version,
                cash=record.cash,
                as_of=record.cash_as_of,
        ...
        for value in record.trades:
            connection.execute(
                ...
                .values(
                    ...
                    source_kind=value.trade.source_kind,
                    ...
                    fees=value.trade.fees,
                    tax=value.trade.tax,
                    executed_at=value.trade.executed_at,
            ...
            for allocation in value.allocations:
                connection.execute(
                    pg_insert(_TRADE_ALLOCATIONS)
                    .values(
                        owner_user_id=actor_id,
                        trade_id=value.trade_id,
                        trade_version=value.version,
        ...
        for value in record.corrections:
            ...
            for allocation in value.reversed_allocations:
                connection.execute(
                    pg_insert(_CORRECTION_ALLOCATIONS)
                    .values(
                        owner_user_id=actor_id,
                        correction_id=value.correction_id,
                        correction_version=value.version,
                        bucket_id=allocation.bucket_id,
                        quantity=allocation.quantity,
                    )
                    .on_conflict_do_nothing()
        ...
        for value in record.company_actions:
            ...
            for bucket_id in set(value.allocation.shares) | set(value.allocation.cash):
                connection.execute(
                    pg_insert(_COMPANY_ACTION_ALLOCATIONS)
                    .values(
                        owner_user_id=actor_id,
                        action_id=value.action_id,
                        action_version=value.version,
                        bucket_id=bucket_id,
        ...
        *,
        ...
        with self._connection() as connection:
            ...
            if receipt is not None:
                ...
                return replay
            current = connection.execute(
                sa.select(_CURRENT_STATE.c.version)
                .where(
            ...
            connection.execute(
                current_insert.on_conflict_do_update(
                    ...
                    set_={
                        "version": current_insert.excluded.version,
                        "cash": current_insert.excluded.cash,
            ...
            connection.execute(
                sa.delete(_HOLDINGS).where(_HOLDINGS.c.owner_user_id == actor_id)
            )
            for value in record.holdings:
                connection.execute(
                    sa.insert(_HOLDINGS).values(
                        owner_user_id=actor_id,
                        bucket_id=value.bucket_id,
                        security_id=value.security_id,
                        quantity=value.quantity,
                        official_close=value.official_close,
            ...
            connection.execute(
                sa.insert(_VERSION_HISTORY).values(
                    owner_user_id=actor_id,
                    version=record.version,
                    action=action,
                    recorded_at=record.updated_at,
            ...
            connection.execute(
                sa.insert(_AUDIT_EVENTS).values(
                    ...
                    policy_version="portfolio-record-v1",
                    build_version=self._build_version,
                )
            )
            connection.execute(

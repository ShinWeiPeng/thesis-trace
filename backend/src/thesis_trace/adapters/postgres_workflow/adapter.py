...
class PostgresWorkflowStore:
    ...
    def for_transaction(
        ...
        connection: Any,
        *,
        security_context_provider: Callable[[], SecurityContext | None] | None = None,
    ...
    def _connection(self) -> Iterator[Any]:
        ...
        if self._transaction_connection is not None:
            if not self._transaction_connection.in_transaction():
                raise ValueError("active_transaction_required")
            connection = self._transaction_connection.connection.driver_connection
            identity = connection.execute(
                "SELECT current_setting('app.user_id',true),current_setting('app.role',true)"
            ).fetchone()
            if identity != (context.user_id, context.role.value):
    ...
    def _row(cursor: Any) -> dict[str, Any]:
        ...
        return dict(
            zip((column.name for column in cursor.description), values, strict=True)
        )
        ...
        *,
        ...
        with self._connection() as connection:
            ...
            connection.execute(
                ...
                (
                    self._lock_key(
                        f"{creation_rule_version}:{item.source_domain}:{item.source_record_id}"
                    ),
                ),
            )
            ...
            inserted = connection.execute(
                ...
                (
                    ...
                    item.reason,
                    item.status.value,
                    item.priority.system_priority.value,
                    item.priority.effective_priority.value,
                    item.priority.safety_floor.value
                    if item.priority.safety_floor
                    else None,
                    item.priority.safety_locked,
                    json.dumps(item.priority.rule_ids),
                    item.priority.policy_version,
    ...
    def get_by_source(
        ...
        with self._connection() as connection:
            cursor = connection.execute(
                ...
                WHERE assignee_user_id=%s AND source_domain=%s AND source_record_id=%s AND source_version=%s
                ORDER BY created_at,item_id LIMIT 1""",
                (actor_id, source_domain, record_id, source_version),
            )
            row = cursor.fetchone()
            if row is None:
                return None
    ...
    def query(self, query: ActionInboxQuery) -> ActionInboxPage:
        ...
        if query.cursor:
            ...
            if (
                not required.issubset(cursor)
                or cursor.get("v") != 1
                or cursor.get("binding") != binding
            ):
        ...
        with self._connection() as connection:
            row = connection.execute(
                ...
                """,
                {
                    ...
                    "last_primary_null": last_primary_null,
                    "last_due": last_due,
                    "last_due_null": last_due_null,
        ...
        if len(rows) > query.page_size:
            ...
            primary = (
                {"critical": 4, "high": 3, "normal": 2, "low": 1}[
                    last["effective_priority"]
                ]
                if query.sort == "effective_priority"
                else last[query.sort]
            )
            next_cursor = self._encode_cursor(
                {
                    ...
                    "as_of": as_of,
                    "primary": primary,
                    "primary_null": primary is None,
                    "due": last["due_at"],
                    "due_null": last["due_at"] is None,
    ...
    def count_audit_events(self, item_id: str) -> int:
        with self._connection() as connection:
            return connection.execute(
                "SELECT count(*) FROM workflow.audit_events WHERE item_id=%s",
                (item_id,),
            ).fetchone()[0]
    ...
    def count_priority_evaluations(self, item_id: str) -> int:
        with self._connection() as connection:
            return connection.execute(
                "SELECT count(*) FROM workflow.action_priority_evaluations WHERE item_id=%s",
                (item_id,),
            ).fetchone()[0]

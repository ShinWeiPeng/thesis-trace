...
_THESES = sa.Table(
    ...
    sa.Column("company_version", sa.Integer),
    sa.Column("status", sa.Text),
    sa.Column("cycle", sa.Integer),
    sa.Column("title", sa.Text),
    sa.Column("narrative", sa.Text),
    sa.Column("reflection_pending", sa.Boolean),
    sa.Column("policy_version", sa.Text),
    sa.Column("created_at", sa.DateTime(timezone=True)),
    sa.Column("updated_at", sa.DateTime(timezone=True)),
    sa.Column("valuation_draft_version", sa.Integer),
    schema="thesis",
)
_VALUATION_DRAFTS = sa.Table(
    ...
    _METADATA,
    sa.Column("thesis_id", sa.Text, primary_key=True),
    sa.Column("draft_version", sa.Integer, primary_key=True),
    sa.Column("owner_user_id", sa.Text),
    sa.Column("payload", sa.JSON),
    sa.Column("saved_at", sa.DateTime(timezone=True)),
    sa.Column("method", sa.Text),
    sa.Column("benchmark_source", sa.Text),
    sa.Column("benchmark_variant", sa.Text),
    sa.Column("target_date", sa.Date),
    sa.Column("expires_at", sa.DateTime(timezone=True)),
    sa.Column("cost_profile_version", sa.Integer),
    sa.Column("policy_version", sa.Text),
    sa.Column("basis_date", sa.Date),
...
_VALUATION_SNAPSHOTS = sa.Table(
    "valuation_snapshots",
    _METADATA,
    sa.Column("valuation_id", sa.Text, primary_key=True),
    sa.Column("version", sa.Integer, primary_key=True),
    sa.Column("thesis_id", sa.Text),
    sa.Column("owner_user_id", sa.Text),
...
_LIFECYCLE_CYCLES = sa.Table(
    ...
    _METADATA,
    sa.Column("thesis_id", sa.Text),
    sa.Column("thesis_version", sa.Integer),
    sa.Column("owner_user_id", sa.Text),
    sa.Column("cycle", sa.Integer),
    sa.Column("status", sa.Text),
    sa.Column("reflection_pending", sa.Boolean),
...
_CONDITIONS = sa.Table(
    ...
    sa.Column("condition_version", sa.Integer),
...
_OUTCOME_EVIDENCE_LINKS = sa.Table(
    ...
    _METADATA,
    sa.Column("outcome_id", sa.Text),
    sa.Column("outcome_version", sa.Integer),
    sa.Column("owner_user_id", sa.Text),
    sa.Column("evidence_id", sa.Text),
    sa.Column("evidence_version", sa.Integer),
    sa.Column("company_id", sa.Text),
...
_REFLECTION_DRAFTS = sa.Table(
    ...
    sa.Column("original_assumption", sa.Text),
    sa.Column("judgment_errors", sa.Text),
    sa.Column("missing_evidence", sa.Text),
...
_REFLECTIONS = sa.Table(
    ...
    _METADATA,
    sa.Column("thesis_id", sa.Text),
    sa.Column("reflection_id", sa.Text),
    sa.Column("version", sa.Integer),
    sa.Column("owner_user_id", sa.Text),
    sa.Column("cycle", sa.Integer),
    sa.Column("original_assumption", sa.Text),
...
_AUDIT_EVENTS = sa.Table(
    ...
    sa.Column("policy_version", sa.Text),
    sa.Column("build_version", sa.Text),
    schema="thesis",
)
_COMMAND_RECEIPTS = sa.Table(
    ...
    sa.Column("command_digest", sa.Text),
    sa.Column("thesis_id", sa.Text),
    sa.Column("thesis_version", sa.Integer),
    schema="thesis",
)
...
class PostgresThesisStore:
    ...
    @staticmethod
    def _lock_inputs(connection: Any, actor_id: str, thesis_id: str) -> None:
        connection.execute(
            "SELECT pg_advisory_xact_lock(hashtextextended(%s,0))",
            (f"recommendation-thesis:{actor_id}:{thesis_id}",),
        )

    def lock_recommendation_inputs(self, actor_id: str, thesis_id: str) -> None:
        context = self._security_context_provider()
        if context is None or context.user_id != actor_id:
        ...
        with self._connection() as connection:
            ...
            if actual is None or tuple(actual) != (context.user_id, context.role.value):
                raise PermissionError("transaction_context_mismatch")
            self._lock_inputs(connection, actor_id, thesis_id)
    ...
    def _valuation_draft(value: dict[str, Any]) -> ValuationDraft:
        def distribution(item: dict[str, Any] | None) -> ValuationDistribution | None:
            ...
            return ValuationDistribution(
                median=Decimal(item["median"]),
                p75=Decimal(item["p75"]),
                coverage=ValuationCoverage(item["coverage"]),
                sample_count=int(item["sample_count"]),
                sorted_samples=tuple(
                    Decimal(child) for child in item.get("sorted_samples", [])
                ),
                ...
                p75_interpolation_fraction=Decimal(
                    item.get("p75_interpolation_fraction", "0")
                ),
        ...
        def sample(item: dict[str, Any]) -> ValuationSample:
            return ValuationSample(
                ...
                Decimal(item["multiple"]),
                Decimal(item["denominator"]),
        ...
        def benchmark(item: dict[str, Any] | None) -> ValuationBenchmarkSnapshot | None:
            ...
            return ValuationBenchmarkSnapshot(
                item["source"],
                resolved,
                tuple(sample(child) for child in item["valid_samples"]),
                tuple(item["exclusions"]),
                date.fromisoformat(item["data_start"]),
                date.fromisoformat(item["data_end"]),
                item["policy_version"],
            )
        ...
        return ValuationDraft(
            ...
            value["benchmark_variant"],
            tuple(value["peer_company_ids"]),
            selected_distribution,
            ValuationValidity(
                date.fromisoformat(validity["target_date"]),
                datetime.fromisoformat(validity["expires_at"]),
                tuple(validity["expiry_causes"]),
                validity["policy_version"],
            ),
            None
            if result is None
            else ValuationReturn(
                Decimal(result["target_price"]),
                Decimal(result["purchase_outflow"]),
                Decimal(result["terminal_inflow"]),
                Decimal(result["annualized_return"]),
                result["tax_disclaimer"],
                result["policy_version"],
            ),
            None if value.get("forecast") is None else Decimal(value["forecast"]),
            Decimal(value["quantity"]),
            Decimal(value["buy_price"]),
            Decimal(value["cash_dividend"]),
            int(value["cost_profile_version"]),
            datetime.fromisoformat(value["saved_at"]),
            benchmark(value.get("company_history")),
            benchmark(value.get("peer_group")),
            None
            if value.get("benchmark_difference_median") is None
            else Decimal(value["benchmark_difference_median"]),
            None
            if value.get("benchmark_difference_p75") is None
            else Decimal(value["benchmark_difference_p75"]),
            tuple(value.get("benchmark_abstentions", [])),
            value.get("source_selection_reason", ""),
            None
            if forecast_source is None
            else PostgresThesisStore._reference(forecast_source),
            None
            if value.get("forecast_confirmed_at") is None
            else datetime.fromisoformat(value["forecast_confirmed_at"]),
            None
            if value.get("next_report_time") is None
            else datetime.fromisoformat(value["next_report_time"]),
            None
            if value.get("material_event_time") is None
            else datetime.fromisoformat(value["material_event_time"]),
            tuple(
                PeerValuationMember(
                    item["company_id"],
                    item["inclusion_reason"],
                    ValuationMethod(item["method"]),
                    bool(item["taiwan_listed"]),
                    bool(item["owner_confirmed"]),
                    sample(item["sample"]),
                )
            ...
            basis_date=None
            if value.get("basis_date") is None
            else date.fromisoformat(value["basis_date"]),
    ...
    def decode_record(cls, value: dict[str, Any]) -> ThesisRecord:
        ...
        valuation_draft = (
            None
            if valuation_draft_value is None
            else cls._valuation_draft(valuation_draft_value)
        )
        valuation_snapshots = tuple(
            ValuationSnapshot(
                item["valuation_id"],
                int(item["version"]),
                int(item["draft_version"]),
                cls._valuation_draft(item["draft"]),
        ...
        return ThesisRecord(
            ...
            evidence_refs=tuple(
                cls._reference(item) for item in value["evidence_refs"]
            ),
            outcome=outcome,
    ...
    def encode_record(record: ThesisRecord) -> dict[str, Any]:
        ...
        outcome = (
            ...
            if record.outcome is None
                ...
                "evidence_refs": [
                    reference(item) for item in record.outcome.evidence_refs
                ],
            }
        )
        reflection = (
        ...
        valuation_draft = (
            ...
            else PostgresThesisStore._valuation_draft_payload(record.valuation_draft)
        )
        valuation_snapshots = [
            {
    ...
    def _get(
        self, connection: Any, actor_id: str, thesis_id: str
    ) -> ThesisRecord | None:
        row = connection.execute(
            sa.select(
                _THESES.c.thesis_id,
                _THESES.c.version,
                _THESES.c.owner_user_id,
                _THESES.c.company_id,
                _THESES.c.company_version,
                _THESES.c.title,
                _THESES.c.narrative,
                _THESES.c.status,
                _THESES.c.cycle,
                _THESES.c.reflection_pending,
                _THESES.c.created_at,
                _THESES.c.updated_at,
                _THESES.c.policy_version,
                _THESES.c.valuation_draft_version,
            ).where(
        ...
        payload: dict[str, Any] = {
            ...
            "owner_user_id": row[2],
            "company_id": row[3],
            "company_version": int(row[4]),
            "title": row[5],
            "narrative": row[6],
            "status": row[7],
            "cycle": int(row[8]),
            "reflection_pending": bool(row[9]),
        ...
        if row[13] is not None:
            draft_row = connection.execute(
                sa.select(
                    _VALUATION_DRAFTS.c.payload,
                    _VALUATION_DRAFTS.c.basis_date,
                    _VALUATION_DRAFTS.c.horizon_months,
                ).where(
            ...
            if draft_row is not None:
                payload["valuation_draft"] = {
                    **draft_row[0],
                    "basis_date": None
                    if draft_row[1] is None
                    else draft_row[1].isoformat(),
                    "horizon_months": draft_row[2],
                }
        snapshot_rows = connection.execute(
            sa.select(
                _VALUATION_SNAPSHOTS.c.valuation_id,
                _VALUATION_SNAPSHOTS.c.version,
                _VALUATION_SNAPSHOTS.c.draft_version,
                _VALUATION_SNAPSHOTS.c.payload,
                _VALUATION_SNAPSHOTS.c.published_at,
                _VALUATION_SNAPSHOTS.c.reason,
                _VALUATION_SNAPSHOTS.c.policy_version,
                _VALUATION_SNAPSHOTS.c.basis_date,
                _VALUATION_SNAPSHOTS.c.horizon_months,
            )
            .where(
        ...
        payload["valuation_snapshots"] = [
            {
                "valuation_id": value[0],
                "version": int(value[1]),
                "draft_version": int(value[2]),
                "draft": {
                    **value[3]["draft"],
                    "basis_date": None if value[7] is None else value[7].isoformat(),
                    "horizon_months": value[8],
                },
                "published_at": value[4].isoformat(),
                "reason": value[5],
        ...
        payload["conditions"] = [
            ...
            for value in connection.execute(
                sa.select(
                    _CONDITIONS.c.condition_id,
                    _CONDITIONS.c.condition_version,
                    _CONDITIONS.c.summary,
                    _CONDITIONS.c.active,
                )
                .where(
                    _CONDITIONS.c.thesis_id == thesis_id,
                    _CONDITIONS.c.thesis_version == row[1],
                )
                .order_by(_CONDITIONS.c.condition_id)
            ).fetchall()
        ]
        payload["evidence_refs"] = [
            ...
            for value in connection.execute(
                sa.select(
                    _EVIDENCE_LINKS.c.evidence_id,
                    _EVIDENCE_LINKS.c.evidence_version,
        ...
        outcome = connection.execute(
            sa.select(
                _OUTCOMES.c.outcome_id,
                _OUTCOMES.c.version,
                _OUTCOMES.c.cycle,
                _OUTCOMES.c.observed_at,
                _OUTCOMES.c.result,
            )
            .where(
            ...
            .order_by(_OUTCOMES.c.version.desc())
            .limit(1)
        ).fetchone()
        if outcome is None:
            payload["outcome"] = None
        else:
            outcome_refs = connection.execute(
                sa.select(
            ...
            payload["outcome"] = {
                ...
                "version": int(outcome[1]),
                "cycle": int(outcome[2]),
                "observed_at": outcome[3].isoformat(),
                "result": outcome[4],
                "evidence_refs": [
                    {
                        "record_type": "evidence",
        ...
        reflection = connection.execute(
            sa.select(
                _REFLECTIONS.c.reflection_id,
                _REFLECTIONS.c.version,
                _REFLECTIONS.c.cycle,
                _REFLECTIONS.c.original_assumption,
                _REFLECTIONS.c.judgment_errors,
                _REFLECTIONS.c.missing_evidence,
                _REFLECTIONS.c.improvement,
            )
            .where(
            ...
            .order_by(_REFLECTIONS.c.version.desc())
            .limit(1)
        ).fetchone()
        payload["reflection"] = (
            None
            if reflection is None
                ...
                "reflection_id": reflection[0],
                "version": int(reflection[1]),
                "cycle": int(reflection[2]),
                "original_assumption": reflection[3],
                "judgment_errors": reflection[4],
                "missing_evidence": reflection[5],
                "improvement": reflection[6],
            }
        )
        draft = connection.execute(
            sa.select(
                _REFLECTION_DRAFTS.c.revision,
                _REFLECTION_DRAFTS.c.cycle,
                _REFLECTION_DRAFTS.c.original_assumption,
                _REFLECTION_DRAFTS.c.judgment_errors,
                _REFLECTION_DRAFTS.c.missing_evidence,
                _REFLECTION_DRAFTS.c.improvement,
            ...
            .order_by(_REFLECTION_DRAFTS.c.revision.desc())
            .limit(1)
        ).fetchone()
        payload["reflection_draft"] = (
            None
            if draft is None
            else {
                "revision": int(draft[0]),
                "cycle": int(draft[1]),
                "original_assumption": draft[2],
                "judgment_errors": draft[3],
                "missing_evidence": draft[4],
                "improvement": draft[5],
    ...
    def list_for_company(self, actor_id: str, company_id: str) -> list[ThesisRecord]:
        with self._connection() as connection:
            rows = connection.execute(
                sa.select(_THESES.c.thesis_id)
                .where(
                    _THESES.c.owner_user_id == actor_id,
                    _THESES.c.company_id == company_id,
                )
                .order_by(_THESES.c.updated_at.desc(), _THESES.c.thesis_id)
            ).fetchall()
            return [
                value
                for row in rows
                if (value := self._get(connection, actor_id, row[0])) is not None
            ]
    ...
    def commit(
        ...
        *,
        ...
        if (
            context is None
            or context.user_id != actor_id
            or record.owner_user_id != actor_id
        ):
            raise PermissionError("resource_unavailable")
        payload = self.encode_record(record)
        with self._connection() as connection:
            self._lock_inputs(connection, actor_id, record.thesis_id)
            connection.execute(
                ...
                connection.execute(
                    sa.insert(_THESES).values(
                        ...
                        valuation_draft_version=(
                            None
                            if record.valuation_draft is None
                            else record.valuation_draft.version
                        ),
                    )
                )
            else:
            ...
            connection.execute(
                sa.insert(_RECORD_VERSIONS).values(
                    thesis_id=record.thesis_id,
                    version=record.version,
                    owner_user_id=actor_id,
                    cycle=record.cycle,
            ...
            connection.execute(
                sa.insert(_LIFECYCLE_CYCLES).values(
                    thesis_id=record.thesis_id,
            ...
            for evidence in record.evidence_refs:
                connection.execute(
                    sa.insert(_EVIDENCE_LINKS).values(
                        thesis_id=record.thesis_id,
                        thesis_version=record.version,
                        owner_user_id=actor_id,
            ...
            if record.outcome is not None:
                ...
                for evidence in record.outcome.evidence_refs:
                    connection.execute(
                        pg_insert(_OUTCOME_EVIDENCE_LINKS)
                        .values(
                            outcome_id=record.outcome.outcome_id,
                            outcome_version=record.outcome.version,
                            owner_user_id=actor_id,
                            evidence_id=evidence.record_id,
                            evidence_version=evidence.version,
                            company_id=evidence.company_id,
            ...
            if record.valuation_draft is not None:
                ...
                connection.execute(
                    pg_insert(_VALUATION_DRAFTS)
                    .values(
                        thesis_id=record.thesis_id,
                        draft_version=draft.version,
                        owner_user_id=actor_id,
                        payload=self._valuation_draft_payload(draft),
                        saved_at=draft.saved_at,
                        method=draft.method.value,
                        benchmark_source=draft.benchmark_source,
                        benchmark_variant=draft.benchmark_variant,
                        target_date=draft.validity.target_date,
                        expires_at=draft.validity.expires_at,
                    ...
                    .on_conflict_do_nothing(
                        index_elements=[
                            _VALUATION_DRAFTS.c.thesis_id,
                            _VALUATION_DRAFTS.c.draft_version,
                        ],
                    )
                )
            for snapshot in record.valuation_snapshots:
                connection.execute(
                    pg_insert(_VALUATION_SNAPSHOTS)
                    .values(
                        ...
                        payload={
                            "draft": self._valuation_draft_payload(snapshot.draft)
                        },
                        published_at=snapshot.published_at,
                        method=snapshot.draft.method.value,
                        ...
                        basis_date=snapshot.draft.basis_date,
                        horizon_months=snapshot.draft.horizon_months,
            ...
            connection.execute(
                sa.insert(_AUDIT_EVENTS).values(
                    event_id=uuid.uuid4(),
                    thesis_id=record.thesis_id,
                    thesis_version=record.version,
                    owner_user_id=actor_id,
            ...
            connection.execute(
                sa.insert(_COMMAND_RECEIPTS).values(

...
from thesis_trace.modules.access.confirmation_challenge.contracts import (
...
from thesis_trace.modules.access.identity_registry.contracts import (
    AccessAccount,
    ProviderIdentity,
...
class PostgresAccessAdapter:
    ...
    def __init__(
        ...
        *,
        recovery_fault_hook: Callable[[], None] | None = None,
        confirmed_mutation_fault_hook: Callable[[], None] | None = None,
    ...
    def confirmation_for_transaction(
        self, connection, *, clock: Callable[[], datetime]
    ) -> AccessConfirmationService:
        if not connection.in_transaction():
            raise ValueError("inactive_transaction")
        bound = PostgresAccessAdapter(self._provider, transaction_connection=connection)
        return AccessConfirmationService(ConfirmationService(bound, clock=clock))

    @contextmanager
    def _challenge_connection(self):
    ...
    @contextmanager
    def recommendation_transaction(self, actor_id: str):
        """Revalidate the admitted Owner and pin its role for one adapter transaction."""
        with database_connection(self._engine) as connection:
            actor, context_provider = self._locked_recommendation_owner(
                connection, actor_id
            )
            yield connection, actor, context_provider

    @staticmethod
    def _locked_recommendation_owner(connection, actor_id: str):
        if not connection.in_transaction():
            raise ValueError("inactive_transaction")
        connection.exec_driver_sql(
            "SELECT set_config('app.user_id',%s,true)", (actor_id,)
        )
        connection.exec_driver_sql("SELECT set_config('app.role','owner',true)")
        request_id = str(uuid.uuid4())
        connection.exec_driver_sql(
            "SELECT set_config('app.request_id',%s,true)", (request_id,)
        )
        row = connection.exec_driver_sql(
            "SELECT user_id,role,status,identity_version FROM access.lock_recommendation_owner(%s)",
            (actor_id,),
        ).fetchone()
        if row is None or row[1] != "owner" or row[2] != "active":
            raise PermissionError("resource_unavailable")
        actor = AuthenticatedActor(str(row[0]), Role.OWNER, int(row[3]))
        context = SecurityContext(actor.actor_id, actor.role, request_id)
        return actor, lambda: context
    ...
    def execute_confirmed_mutation(
        ...
        *,
        ...
        with database_connection(
            self._engine, isolation_level="SERIALIZABLE"
        ) as connection:
            connection.exec_driver_sql(
                "SELECT set_config('app.user_id',%s,true)", (actor_id,)
            )
            connection.exec_driver_sql(
                "SELECT set_config('app.role',%s,true)", (actor_role.value,)
            )
            connection.exec_driver_sql(
                "SELECT set_config('app.request_id',%s,true)", (str(uuid.uuid4()),)
            )
            row = connection.exec_driver_sql(
    ...
    def find_mapping(
        ...
        return (
            None
            if row is None
            else ProviderIdentityMapping(
                identity, str(row[0]), bool(row[1]), int(row[2])
            )
        )
    ...
    def get_account(self, user_id: str) -> AccessAccount | None:
        with _redacted_connection(self._provider) as connection:
            row = connection.execute(
                "SELECT user_id::text,role,status,identity_version FROM access.users WHERE user_id=%s",
                (user_id,),
            ).fetchone()
        return (
            None
            if row is None
            else AccessAccount(str(row[0]), Role(row[1]), str(row[2]), int(row[3]))
        )
    ...
    def list_account_summaries(self) -> list[AccountSummary]:
        ...
        return [
            AccountSummary(str(r[0]), Role(r[1]), str(r[2]), str(r[3]), int(r[4]))
            for r in rows
        ]

    def create_account(
        self, *, role: Role, provider_identity: ProviderIdentity, email_fact: str
    ) -> AccessAccount:
        user_id, identity_id = uuid.uuid4(), uuid.uuid4()
        with _redacted_connection(self._provider) as connection:
            with connection.transaction():
                connection.execute(
                    "INSERT INTO access.users(user_id,role,status) VALUES(%s,%s,'active')",
                    (user_id, role.value),
                )
                connection.execute(
                    ...
                    (
                        ...
                        provider_identity.provider_type,
    ...
    def save_session(self, session: AccessSession, token_digest: str) -> None:
        with _redacted_connection(self._provider) as connection:
            identity = connection.execute(
                ...
                (
                    session.user_id,
                    session.provider_identity.provider_id,
                    session.provider_identity.provider_type,
                    session.provider_identity.subject,
                ),
            ).fetchone()
            if identity is None:
                raise PermissionError("resource_unavailable")
            connection.execute(
                ...
                (
                    ...
                    session.expires_at,
                    session.expires_at,
                    session.identity_version,
                    session.recovery_policy_version,
                ),
            )
    ...
    def save_recovery_session(
        ...
        if (
            session.provider_identity.provider_type != "cloudflare"
            or not reason.strip()
        ):
            raise PermissionError("resource_unavailable")
        event_id = uuid.uuid5(
            uuid.NAMESPACE_URL, f"access-recovery:{session.session_id}"
        )
        with _redacted_connection(self._provider) as connection:
            with connection.transaction():
                identity = connection.execute(
                ...
                connection.execute(
                    ...
                    (
                        ...
                        json.dumps(
                            {
                                "request_type": "disable_recovery_access",
    ...
    def revoke_recovery_sessions(
        self, actor_id: str, reason: str, policy_version: str, when: datetime
    ) -> int:
        event_id = uuid.uuid4()
        with _redacted_connection(self._provider) as connection:
            with connection.transaction():
    ...
    def get_session(self, token_digest: str) -> AccessSession | None:
        ...
        return AccessSession(
            str(row[0]),
            str(row[1]),
            ProviderIdentity(str(row[2]), str(row[3]), str(row[4])),
            row[5],
            bool(row[6]),
            row[7],
            int(row[8]),
    ...
    def revoke_session(self, session_id: str, when: datetime) -> None:
        with _redacted_connection(self._provider) as connection:
            connection.execute(
                "UPDATE access.sessions SET revoked_at=%s WHERE session_id=%s AND revoked_at IS NULL",
                (when, session_id),
            )

    def save_challenge(
        self, challenge: ConfirmationChallenge, token_digest: str
    ) -> None:
        with self._challenge_connection() as connection:
            connection.exec_driver_sql(
                ...
                (
                    ...
                    json.dumps(
                        challenge.impact_summary, sort_keys=True, separators=(",", ":")
                    ),
                    token_digest,
                    challenge.expires_at - timedelta(minutes=5),
            ...
            with connection.begin_nested():
                ...
                connection.exec_driver_sql(
                    ...
                    (
                        ...
                        reason,
                        json.dumps(
                            {"challenge_id": row[0], "payload_digest": payload_digest}
                        ),
                    ),
                )
    ...
    def execute_confirmed_account_action(
        self,
        *,
        token: str,
        actor_id: str,
            ...
            with connection.transaction():
                ...
                if action_type == "change_role":
                    ...
                    disabled = connection.execute(
                        ...
                        (
                            row[1],
                            payload[f"{prefix}provider_id"],
                            payload[f"{prefix}provider_type"],
                            payload[f"{prefix}provider_subject"],
                        ),
                    ).fetchone()
                    changed = (
                        None
                ...
                if changed is None:
                    raise PermissionError("resource_unavailable")
                connection.execute(
                    "UPDATE access.confirmation_challenges SET consumed_at=%s,result_id=%s WHERE challenge_id=%s",
                    (now, row[1], row[0]),
                )
                connection.execute(
                    """INSERT INTO access.security_audit_events(event_id,actor_user_id,subject_id,action,reason,metadata)
    ...
    def execute_confirmed_action(
        ...
        *,
        ...
        result = next(
            (
                item
                for item in self.list_account_summaries()
                if item.user_id == self._challenge_result(challenge_id)
            ),
            None,
        )
    ...
    def _challenge_result(self, challenge_id: str) -> str:
        with _redacted_connection(self._provider) as connection:
            row = connection.execute(
                "SELECT result_id FROM access.confirmation_challenges WHERE challenge_id=%s",
                (challenge_id,),
            ).fetchone()
    ...
    def _challenge_id_for_result(self, result_id: str) -> str:
        with _redacted_connection(self._provider) as connection:
            row = connection.execute(
                "SELECT challenge_id::text FROM access.confirmation_challenges WHERE result_id=%s ORDER BY consumed_at DESC LIMIT 1",
                (result_id,),
            ).fetchone()
        if row is None:
            raise PermissionError("resource_unavailable")
    ...
    def execute_confirmed_account_creation(
        self,
        *,
        token: str,
        actor_id: str,
            ...
            with connection.transaction():
                ...
                connection.execute(
                    ...
                    (
                        ...
                        normalized["email_fact"],
                    ),
                )
                connection.execute(
                    "UPDATE access.confirmation_challenges SET consumed_at=%s,result_id=%s WHERE challenge_id=%s",
                    (now, user_id, challenge[0]),
                )
                connection.execute(
                    """INSERT INTO access.security_audit_events(event_id,actor_user_id,subject_id,action,reason,metadata)
                    ...
                    (
                        ...
                        reason,
                        json.dumps(
                            {"challenge_id": challenge[0], "payload_digest": digest}
                        ),
                    ),
                )
            ...
            with connection.transaction():
                ...
                connection.execute(
                    ...
                    (
                        ...
                        actor_id,
                        session_id,
                        json.dumps({"outbox_event_id": str(event_id)}),
                    ),
                )
        return True

...
def test_postgres_portfolio_versions_receipts_and_rls_projection() -> None:
    ...
    cash = service.save_investable_cash(
        SaveInvestableCashCommand(
            actor, profile.version, Decimal("100000"), NOW, "Save cash", "cash-1"
        ),
        now=NOW,
    )
    ...
    with psycopg.connect(url, autocommit=True) as connection:
        ...
        with connection.transaction():
            ...
            assert (
                connection.execute(
                    "SELECT count(*) FROM portfolio.current_state"
                ).fetchone()[0]
                == 1
            )
        with connection.transaction():
...
def _configured(url: str) -> tuple[PortfolioService, PostgresPortfolioStore, object]:
    ...
    profile = service.save_cost_profile(
        SaveCostProfileCommand(
            PortfolioActorContext(OWNER_ID, True),
            0,
            Decimal("0"),
            Decimal("0"),
            Decimal("0"),
    ...
    portfolio = service.save_investable_cash(
        SaveInvestableCashCommand(
            PortfolioActorContext(OWNER_ID, True),
            profile.version,
            Decimal("100000"),
...
def test_confirmed_trade_consumes_challenge_and_portfolio_atomically() -> None:
    ...
    confirmations = ConfirmationService(
        access, clock=lambda: NOW, token_factory=lambda: f"trade-token-{uuid.uuid4()}"
    )
    payload = {
    ...
    changed = coordinator.execute(
        actor=AuthenticatedActor(OWNER_ID, Role.OWNER, 1),
        preview=preview,
        reason="Confirm",
        idempotency_key="trade-atomic",
    ...
    with psycopg.connect(url) as connection:
        ...
        audit = connection.execute(
            ...
            WHERE owner_user_id=%s AND action='trade_confirmed'""",
            (OWNER_ID,),
        ).fetchone()
...
def test_confirmed_trade_fault_rolls_back_challenge_and_portfolio() -> None:
    ...
    access = PostgresAccessAdapter(
        lambda: url,
        confirmed_mutation_fault_hook=lambda: (_ for _ in ()).throw(
            RuntimeError("fault")
        ),
    )
    confirmations = ConfirmationService(
        access,
        clock=lambda: NOW,
        token_factory=lambda: f"trade-rollback-{uuid.uuid4()}",
    )
    payload = {
        "fingerprint": preview.fingerprint,
        "preview_digest": preview.preview_digest,
    ...
    with pytest.raises(PostgresUnavailable, match="postgres_unavailable"):
        PostgresConfirmedPortfolioTrade(access, store, service).execute(
            ...
            preview=preview,
            reason="Confirm",
            idempotency_key="trade-rollback",
            challenge_token=token,
            now=NOW,
        )
...
def test_confirmed_correction_and_company_action_are_atomic_append_only_records() -> (
    None
):
    url = os.environ["THESIS_TRACE_TEST_DATABASE_URL"]
    ...
    confirmations = ConfirmationService(
        access,
        clock=lambda: NOW,
        token_factory=lambda: f"portfolio-mutation-{uuid.uuid4()}",
    )
    coordinator = PostgresConfirmedPortfolioTrade(access, store, service)

    trade_payload = {
        "fingerprint": trade_preview.fingerprint,
        "preview_digest": trade_preview.preview_digest,
    }
    trade_token, _ = confirmations.preview(
        OWNER_ID,
        "confirm_portfolio_trade",
        OWNER_ID,
        trade_preview.expected_portfolio_version,
        trade_payload,
        {"consequences": ["trade"]},
    )
    traded = coordinator.execute(
        ...
        reason="Confirm",
        idempotency_key="trade-before-action",
        challenge_token=trade_token,
        now=NOW,
    )
    ...
    action_payload = {
        "preview_digest": action_preview.preview_digest,
        "security_id": "2330",
    }
    action_token, _ = confirmations.preview(
        ...
        OWNER_ID,
        action_preview.expected_portfolio_version,
        action_payload,
        {"consequences": ["action"]},
    )
    ...
    confirmations = ConfirmationService(
        access,
        clock=lambda: NOW,
        token_factory=lambda: f"portfolio-correction-{uuid.uuid4()}",
    )
    coordinator = PostgresConfirmedPortfolioTrade(access, store, service)
    trade_payload = {
        "fingerprint": trade_preview.fingerprint,
    ...
    traded = coordinator.execute(
        ...
        preview=trade_preview,
        reason="Confirm",
        idempotency_key="trade-before-correction",
        challenge_token=trade_token,
        now=NOW,
    )
    correction_preview = service.preview_correction(
        PortfolioActorContext(OWNER_ID, True),
        expected_version=traded.version,
        original_trade_id=traded.trades[0].trade_id,
        now=NOW,
    )
    correction_payload = {
        "original_trade_id": correction_preview.original_trade_id,
        "preview_digest": correction_preview.preview_digest,
    }
    correction_token, _ = confirmations.preview(
        OWNER_ID,
        "correct_portfolio_trade",
        OWNER_ID,
        correction_preview.expected_portfolio_version,
        correction_payload,
    ...
    corrected = coordinator.execute_correction(
        ...
        reason="Broker corrected",
        idempotency_key="correction-atomic",
        challenge_token=correction_token,
        now=NOW,
    )

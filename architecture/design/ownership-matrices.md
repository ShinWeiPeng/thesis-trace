...
| Canonical source identities and immutable content snapshots | `evidence_collection` | `postgres_research_adapter` | Durable database state | Collection independently creates version-bound `(normalization_policy_version, canonical_url)` identities and globally content-hash-deduplicated snapshots, then binds each observation to the identity and the content actually retrieved in the same transaction. Historical identities are immutable and are never reinterpreted under a newer policy. |
...
| `CollectedSourceSnapshot` | `evidence_collection` | L2 | Immutable source result: canonical URL, normalization policy version, publisher, content hash, times, excerpt, source category, lineage | Public; aggregate/event lifetime; immutable | Collection validates/creates; Evidence owner persists | `EvidenceUrl` | PostgreSQL persists the policy version on both the canonical-source identity and the snapshot that first materialized the content; `(normalization_policy_version, canonical_url)` is the URL dedup key while content hash remains globally deduplicated; raw bytes and HTTP representations are excluded |

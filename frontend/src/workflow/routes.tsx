...
function queryFrom(path: URL): ActionInboxQuery {
  ...
  return {
    ...
    item_type: source.get("item_type"), status: source.get("status"), priority: source.get("priority"),
    created_from: source.get("created_from"), created_to: source.get("created_to"),
    due_from: source.get("due_from"), due_to: source.get("due_to"),
...
function Detail({ item, client, onChanged, returnQuery }: {
  ...
  onChanged(item: ActionItemResponse): Promise<void>; returnQuery: string;
}) {
  ...
  const transition = async (target: ActionItemTransitionBody["target_status"]) => {
    ...
    try {
      ...
      await onChanged(changed); setReason("");
  ...
  return <article className="action-detail" aria-labelledby="action-detail-title">
    ...
    <a className="secondary-button context-link" href={`/companies/${encodeURIComponent(item.company_id)}?fromAction=${encodeURIComponent(item.item_id)}&sourceDomain=${encodeURIComponent(item.source_domain)}&sourceRecord=${encodeURIComponent(item.source_record_id)}&sourceVersion=${item.source_version}&return=${encodeURIComponent(returnQuery)}`}>開啟公司脈絡</a>
...
export function ActionInboxRoutes({ client = workflowClient, initialPath }: { client?: WorkflowClient; initialPath?: string }) {
  ...
  const updateRoute = (next: ActionInboxQuery) => {
    if (initialPath === undefined) history.replaceState(null, "", `/actions?${queryString(next)}`);
  };
  ...
  useEffect(() => {
    ...
    const scroll = Number(new URLSearchParams(encoded ?? path.search).get("scroll"));
    if (Number.isFinite(scroll) && scroll > 0) scrollTo({ top: scroll });
  }, [page, path, selectedId]);
  const apply = (next: ActionInboxQuery) => { setQuery(next); updateRoute(next); void load(next); };
  const applySearch = (event: FormEvent) => { event.preventDefault(); apply({ ...query, search: search.trim() || null, cursor: null }); };
  ...
  const summaryFilter = (kind: "urgent" | "today" | "deferred" | "open") => {
    ...
    apply({ ...query, cursor: null, priority: kind === "urgent" ? "urgent" : null,
      status: kind === "deferred" ? "deferred" : null, open_only: kind !== "deferred" ? true : query.open_only,
  ...
  return <main className="action-page"><header className="action-page-header"><div><p className="eyebrow">Workflow</p><h1>Action Inbox</h1><p>由伺服器計算優先級、合法操作與一致摘要。</p></div><form className="action-search" role="search" onSubmit={applySearch}><label htmlFor="action-search">搜尋公司或理由</label><div><input id="action-search" value={search} onChange={(event) => setSearch(event.target.value)} /><button className="primary-button">搜尋</button></div></form></header>
    {error && <p role="alert" className="error-banner">{error}</p>}
    {page && <section className="action-summary" aria-label="待辦摘要"><button onClick={() => summaryFilter("urgent")}>緊急 {page.summary.urgent}</button><button onClick={() => summaryFilter("today")}>今日到期 {page.summary.due_today}</button><button onClick={() => summaryFilter("deferred")}>已延後 {page.summary.deferred}</button><button onClick={() => summaryFilter("open")}>全部待處理 {page.summary.all_open}</button></section>}
    <section className="action-filters" aria-label="待辦篩選"><label>事項類型<select value={query.item_type ?? ""} onChange={(event) => apply({ ...query, item_type: event.target.value || null, cursor: null })}><option value="">全部</option><option value="anomaly_review">Anomaly review</option></select></label><label>狀態<select value={query.status ?? ""} onChange={(event) => apply({ ...query, status: event.target.value || null, cursor: null })}><option value="">全部</option>{Object.entries(statusLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label>優先級<select value={query.priority ?? ""} onChange={(event) => apply({ ...query, priority: event.target.value || null, cursor: null })}><option value="">全部</option><option value="urgent">critical + high</option><option value="critical">critical</option><option value="high">high</option><option value="normal">normal</option><option value="low">low</option></select></label><label>排序<select value={query.sort} onChange={(event) => apply({ ...query, sort: event.target.value, cursor: null })}><option value="effective_priority">優先級</option><option value="due_at">到期時間</option><option value="created_at">建立時間</option><option value="updated_at">更新時間</option></select></label><label>方向<select value={query.direction} onChange={(event) => apply({ ...query, direction: event.target.value, cursor: null })}><option value="desc">遞減</option><option value="asc">遞增</option></select></label><label className="check-field"><input type="checkbox" checked={query.open_only} onChange={(event) => apply({ ...query, open_only: event.target.checked, cursor: null })} />僅未結項</label></section>
    <div className={`action-master-detail${selectedId ? " has-detail" : ""}`}><section className="action-list" aria-label="待辦清單">{!page ? <p>載入中…</p> : page.items.length === 0 ? <p className="empty-state">目前沒有符合條件的待辦。</p> : page.items.map((item) => <a className={`action-row${selectedId === item.item_id ? " selected" : ""}`} key={item.item_id} href={`/actions/${encodeURIComponent(item.item_id)}?return=${encodeURIComponent(returnContext)}`}><span className={`priority-dot priority-${item.effective_priority}`} aria-label={`${item.effective_priority} 優先級`} /><span><strong>{item.company_ticker} · {item.company_name}</strong><small>{item.item_type} · {item.reason}</small><small>{item.created_at}{item.due_at ? ` · 到期 ${item.due_at}` : ""}</small></span><span className={`action-status status-${item.status}`}>{statusLabels[item.status]}</span></a>)}{page?.next_cursor && <button className="secondary-button load-more" onClick={() => apply({ ...query, cursor: page.next_cursor })}>載入下一頁</button>}</section>
      {selectedId && <aside className="action-detail-pane">{detail ? <Detail item={detail} client={client} returnQuery={returnContext} onChanged={async (changed) => { setDetail(changed); await load(query); }} /> : <p>載入明細…</p>}</aside>}
    </div>
  </main>;
}
